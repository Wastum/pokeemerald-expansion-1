...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_INTERVIEWER] = { _("Interview"), 12 },
    ...
    [TRAINER_CLASS_GENTLEMAN] = { _("Gentle Man"), 20, BALL_LUXURY },
    ...
    //[TRAINER_CLASS_SINNOH_TRAINER] = { _("SinnohTrainer") },
};
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        // Set for level scaling. HighestPartyLevel starts at 1 to prevent
        // glitches occuring if something goes wrong with the variable.
        u8 highestPartyLevel = 1;
        for (i = 0; i < monsCount; i++)
        {
            if (gPlayerParty[i].level > highestPartyLevel)
            {
                highestPartyLevel = gPlayerParty[i].level;
            }
        }
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            u8 trueLevel = (WSTM_LEVEL_SCALE_TRAINER 
                && gMapHeader.regionMapSectionId != MAPSEC_ROUTE_104 
                && gMapHeader.regionMapSectionId != MAPSEC_ROUTE_102
                && gMapHeader.regionMapSectionId != MAPSEC_PETALBURG_WOODS) ? highestPartyLevel : partyData[monIndex].lvl;
            //CreateMon(&party[i], partyData[monIndex].species, partyData[monIndex].lvl, 0, TRUE, personalityValue, otIdType, fixedOtId);
            CreateMon(&party[i], partyData[monIndex].species, trueLevel, 0, TRUE, personalityValue, otIdType, fixedOtId);
